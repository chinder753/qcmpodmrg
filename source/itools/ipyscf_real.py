...
class iface:
    ...
    def local(self):
        ...
        if self.iflowdin:
            ...
            if self.nalpha == self.nbeta:
                ...
                cOrbs = self.mo_coeff[:, self.nfrozen:int(self.nelec / 2)]
                vOrbs = self.mo_coeff[:, int(self.nelec / 2):]
                uc = PM(self.mol, cOrbs).kernel().transpose()
                uv = PM(self.mol, vOrbs).kernel().transpose()
                clmo = cOrbs @ uc
                vlmo = vOrbs @ uv
                ...
                uc = PM(self.mol, cOrbs).kernel().transpose()
                uo = PM(self.mol, oOrbs).kernel().transpose()
                uv = PM(self.mol, vOrbs).kernel().transpose()
                clmo = cOrbs @ uc
                olmo = oOrbs @ uo
                vlmo = vOrbs @ uv
    ...
    def dump(self, fname='mole.h5'):
        ...
        for i in range(int(self.nalpha - self.nfrozen)):
            occun[2 * i] = 1.0
        for i in range(int(self.nbeta - self.nfrozen)):
            occun[2 * i + 1] = 1.0
        print()
        print(('initial occun for', len(occun), ' spin orbitals:\n', occun))
        # sorder = numpy.array([[2 * i, 2 * i + 1] for i in order]).flatten()
        sorder = numpy.array([i for i in order]).flatten()
